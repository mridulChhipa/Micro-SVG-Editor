...
void Canvas::BuildPointPath(QPainterPath& path, const PointContainer& points,
                            bool close) {
  bool first = true;
...
void Canvas::BuildShapePath(QPainterPath& path,
  ...
  for (const auto& [command, points] : s->commands) {
    ...
    if (command == 'M')
      ...
      path.cubicTo(QPointF(points[0].first, points[0].second),
                   QPointF(points[1].first, points[1].second),
                   QPointF(points[2].first, points[2].second));
    else if (command == 'Q')
      path.quadTo(QPointF(points[0].first, points[0].second),
                  QPointF(points[1].first, points[1].second));
    else if (command == 'H')
