...
void Canvas::TextEdit(bool& edited) {
  auto text_obj = std::dynamic_pointer_cast<Text>(
      *std::find(svg_.objects.begin(), svg_.objects.end(), selected_shape_));
  if (text_obj) {
    bool ok[3] = {false, false, false};
    QString new_text = QInputDialog::getText(
        this, "Edit Text", "Enter new text:", QLineEdit::Normal,
        QString::fromStdString(text_obj->content), &ok[0]);
    int new_font_size = QInputDialog::getInt(
        this, "Edit Text", "Enter new Font Size:", text_obj->font_size, 1, 100,
        1, &ok[1]);
    QString new_font_fam = QInputDialog::getText(
        this, "Edit Text", "Enter new Font Family:", QLineEdit::Normal,
        QString::fromStdString(text_obj->font_family), &ok[2]);
